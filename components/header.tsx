    ...
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 border-b border-border">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
        <Link href="/" className="flex items-center gap-2">
          <LogoIcon className="h-10 w-10 md:h-12 md:w-16" />
        ...
        <nav className="hidden items-center gap-0.5 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              href={link.href}
              className={cn(
                "rounded-md px-2.5 py-2 text-xs font-medium transition-colors whitespace-nowrap",
                isActive(link.href)
                  ? "text-primary"
      ...
      {mobileOpen && (
        <div className="border-t border-border bg-background px-4 py-4 lg:hidden animate-slide-down">
